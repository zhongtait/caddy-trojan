...
- Admin API 仅 `127.0.0.1:2019`；Release 包校验 `SHA256SUMS`
...
# 可选：启用全局 sysctl / limits 调优（默认不修改）
sudo bash easytrojan.sh install --domain yourdomain.com --tune-system
```
...
> 只下载入口脚本时，会从最新 Release 下载并校验完整 `easytrojan_bundle.tar.gz`；完整 clone 时优先使用本地 `lib/`。
...
默认安装不会修改全局 sysctl；需要时显式添加 `install --tune-system`。
...
`enable` 会确保 python3≥3.8（缺失则自动安装）、安装 `easytrojan-hub` 与 systemd 单元、生成 token、注入 Caddy 反代，并尽量注册本机用户（可用 `--name` 自定义显示名）。Hub 使用独立的 `easytrojan-hub` 系统用户运行；节点状态损坏时会返回错误并保留 `.bak`，不会静默清空节点。
...
确认机器能访问 GitHub Release，并检查 `easytrojan_bundle.tar.gz` 与 `SHA256SUMS` 是否存在；完整 clone 时也可以直接放置仓库目录。
...
非交互：`sudo bash uninstall.sh -y`。卸载只删除带 EasyTrojan 标记的 Caddy 资源；检测不到标记时会保留现有 Caddy 服务、二进制和 `/etc/caddy`。
...
| `uninstall.sh` | 卸载 |
| `sha` | 上游 commit 标记；用于检测变化并固定对应 Release 构建 |
